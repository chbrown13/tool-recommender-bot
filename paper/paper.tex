...
\begin{figure*}
	...
	\caption{Flowchart presenting our implementation of \tool~for this evaluation: \textit{A.1) Setup for \tool~on virtual machines; A.2) Setup Jenkins server and create \tool~jobs; A.3) Clone GitHub repository; B) Check for code changes; C.1) Add \textsc{Error Prone} tool plugin to pom.xml file, build original code with Maven, and collect output; C.2) Add \textsc{Error Prone} tool plugin to pom.xml file, build modified code with Maven, and collect output; D) Check for bugs fixed between versions; E) Search for other instances of bug; F) Send email to researchers to review; G) Recommend Error Prone; H) Repeat process every 15 minutes; I) Pull latest code from GitHub. }}	
	\label{fig:chartB} 
\end{figure*}
