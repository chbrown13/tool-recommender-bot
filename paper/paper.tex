...
We used real-world open source software applications from GitHub to evaluate \tool. To choose projects for our study from the millions of GitHub repositories, we selected projects that met the following criteria:

\begin{itemize}
\item primariy written in Java,
\item ranked as a 1000 forked repositoy for a month in the last 10 years,
\item builds with Maven, and
\item do not already utilize \textsc{Error Prone}.
\end{itemize}

\textsc{Error Prone} only analyzes Java code, so our evaluation was limited to Java projects. We chose the most forked repositories to sample the most popular GitHub projects in terms of developer activity. Additionally, GitHub suggests forking a repository as a best practice for contributing and making pull requests to projects\footnote{https://guides.github.com/activities/forking/}. Maven is a build automation and dependency management system for Java projects. \tool~requires Maven to automatically modify \textit{pom.xml} files by adding the \textsc{Error Prone} plug-in and running the static analysis tool. Using a GitHub search API with the above constraints, we identified 2003 projects to use for our evaluation.\footnote{\url{https://docs.google.com/spreadsheets/d/1MRmVPiUeDlYMa0UODzMmFbGeYBbahzdeEv1YSgqC1oo/edit?usp=sharing}}
